...
"""
Benchmark Script for Alternative Model (OPT-125M)

This script:
1. Loads the Meta OPT-125M model (as an alternative to Qwen 0.5B)
...
# Constants
MODEL_ID = "facebook/opt-125m"  # Using OPT-125M as an alternative to Qwen 0.5B
SAMPLE_PROMPTS = [
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Benchmark Quantized OPT-125M Model")
    parser.add_argument(
        "--output_dir", 
        type=str, 
        default="./opt_125m_results",
...
def generate_report(benchmark_results, output_dir):
    ...
    with open(report_path, "w") as f:
        f.write("# OPT-125M Quantization Benchmark Report\n\n")
        
        f.write("## Summary\n\n")
        f.write("This report presents the benchmarking results for the OPT-125M model under different quantization schemes.\n\n")
        ...
        f.write("\n## Comparison with Qwen 0.5B\n\n")
        f.write("This benchmark uses Meta's OPT-125M as an alternative to Qwen 0.5B due to compatibility issues with the Qwen model. ")
        f.write("While the models have different architectures and parameter counts, the quantization techniques and benchmarking methodology remain the same, ")
        f.write("allowing for valid comparisons of the effects of different quantization methods on model performance.\n")
