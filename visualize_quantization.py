...
def create_metric_miniplot(ax, models, methods, values, title, cmap='viridis'):
    ...
    for i in range(len(unique_models)):
        for j in range(len(unique_methods)):
            if data[i, j] > 0:
                # Fix for color check - use pyplot's cm directly and handle tuples
                try:
                    # Use pyplot's colormaps directly instead of get_cmap
                    colormap = plt.colormaps[cmap]
                    color_value = colormap(data[i, j]/data.max())
                    # Check if it's a tuple/array and get the first 3 elements (RGB)
                    if hasattr(color_value, '__len__') and len(color_value) >= 3:
                        # Calculate luminance from RGB
                        luminance = 0.299 * color_value[0] + 0.587 * color_value[1] + 0.114 * color_value[2]
                        text_color = 'white' if luminance < 0.5 else 'black'
                    else:
                        text_color = 'black'  # default
                except Exception:
                    text_color = 'black'  # default fallback
...
def create_method_heatmap(ax, results):
    ...
    for i in range(len(methods)):
        for j in range(len(metrics)):
            if counts[i, j] > 0:
                # Calculate text color based on normalized value
